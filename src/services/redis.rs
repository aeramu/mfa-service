...
impl RedisService {
    ...
    pub async fn check_rate_limit(
        ...
        // 3. Calculate new cooldown: base_delay * multiplier^(count - 1)
        // count=1 -> base_delay, count=2 -> base_delay * multiplier, etc.
        let exp = (count.saturating_sub(1)).min(10) as i32;
        let next_cooldown = (base_delay as f64 * multiplier.powi(exp)) as u64;
        
        // 4. Apply the cooldown lock
        let _: () = conn.set_ex(&cooldown_key, "1", next_cooldown).await?;
